...
def test_spd_sqrt():
    ...
    sqrt_B, sqrt_B_inv = projnormal.linalg.spd_sqrt(B, return_inverse=True)
    ...
    assert projnormal.matrix_checks.is_symmetric(sqrt_B), \
        "SPD square root is not symmetric."
    assert projnormal.matrix_checks.is_symmetric(sqrt_B_inv), \
