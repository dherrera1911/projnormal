...
def test_sampling_works(gaussian_parameters):
    ...
    prnorm = ProjectedNormal(n_dim=n_dim, mu=mu, covariance=covariance)
    ...
    assert torch.allclose(norm_samples, torch.tensor(1.0)), 'Samples norm is not 1'
...
def test_moments_work(gaussian_parameters):
...
def test_moment_matching_works(gaussian_parameters, cov_param):
    ...
    prnorm_data = ProjectedNormal(n_dim=n_dim, mu=mu, covariance=covariance,
    ...
    prnorm_fit = ProjectedNormal(n_dim=n_dim, covariance=covariance_initial,
                             covariance_parametrization=cov_param)
